...
enum ConsoleColor {
    ...
    GREEN = 10,     // success
    YELLOW = 14,    // prompts
    BLUE = 9,       // info headers / background
...
// Function to set background + foreground color
void setBackground(ConsoleColor bg, ConsoleColor fg = WHITE) {
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), (bg << 4) | fg);
}
#else
#define setColor(c)
#define setBackground(bg, fg)
#endif
...
// ================= FUNCTION PROTOTYPES =================
bool login();
void showCoverPage();          // Cover page function
void addPatient();
void viewPatient();
void updatePatient();
void deletePatient();
...
void showCoverPage() {
    ...
    // Fill console with background color
    setBackground(BLUE, WHITE);
    for(int i = 0; i < 25; i++) {
        cout << string(80, ' ') << endl;
    }
    ...
    setColor(CYAN);
    cout << "Project by: Group 2\n";
    cout << "Course: C++ Programming 2\n";
    cout << "Instructor: Akede Abera\n";
    cout << "Date: Jan 2026\n\n";
...
int main() {
    ...
    // Show cover page with background
    showCoverPage();
    ...
    do {
        #ifdef _WIN32
        setBackground(BLUE, WHITE); // Keep blue background for menu
        system("cls");
        #endif
        ...
        switch(choice) {
            ...
            case 7: searchPatientByAge(); break;
            case 8: bmiStatsSummary(); break;
            case 9:
                setColor(GREEN);
...
// ================= DATA VALIDATION =================
bool validatePatient(Patient p) {
    if(p.id <= 0) { setColor(RED); cout << "ID must be positive!\n"; setColor(WHITE); return false; }
    if(p.age <= 0 || p.age > 120) { setColor(RED); cout << "Invalid age!\n"; setColor(WHITE); return false; }
    if(p.bmi < 10 || p.bmi > 50) { setColor(RED); cout << "BMI seems incorrect!\n"; setColor(WHITE); return false; }
    if(strlen(p.name) == 0) { setColor(RED); cout << "Name cannot be empty!\n"; setColor(WHITE); return false; }
    if(p.sex != 'M' && p.sex != 'F'&& p.sex != 'f'&& p.sex != 'm') { setColor(RED); cout << "Sex must be M or F!\n"; setColor(WHITE); return false; }
    return true;
}
...
void bmiStatsSummary() {
    ...
    while(file.read((char*)&p, sizeof(Patient))) {
        if(p.active) {
            if(p.bmi < 18.5) underweight++;
            else if(p.bmi < 25) normal++;
            else if(p.bmi < 30) overweight++;
            else obese++;
        }
    }
