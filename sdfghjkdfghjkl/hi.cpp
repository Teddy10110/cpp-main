...
/* ================= MASKED PIN INPUT ================= */
string getMaskedInput(int lang) {
...
bool withdrawCash(User &u, int &atmCash, int lang) {
    fullScreenBackground();
    int amount;
...
bool depositCash(User &u, int lang) {
    fullScreenBackground();
    int amount;
...
bool showBalance(const User &u, int lang) {
    fullScreenBackground();
    if (lang == 1) {
...
bool showTransactionHistory(const User &u, int lang) {
    fullScreenBackground();
...
bool transferFunds(User &sender, User users[], int userCount, int lang) {
    fullScreenBackground();
...
bool changePIN(User &u, int lang) {
    fullScreenBackground();
    int oldPin, newPin;
...
bool registerNewUser(User users[], int &userCount, int lang) {
    fullScreenBackground();
    User newUser;
...
bool resetUserPIN(User users[], int userCount, int lang) {
    fullScreenBackground();
    string accountNumber;
    int newPin;
...
bool loadATMCash(int &atmCash, int lang) {
    fullScreenBackground();
    int amount;
...
/* ================= USER SESSIONS ================= */
bool runCardSession(User users[], int &userCount, int &atmCash, int lang) {
    fullScreenBackground();
    cout << (lang == 1 ? "Please insert your card...\n" : "እባክዎን ካርድዎን ያስገቡ...\n");
    
    string enteredPin = getMaskedInput(lang );
    ...
    do {
        ...
        switch(choice) {
            case 1: continueSession = showBalance(users[userIndex], lang); break;
            case 2: continueSession = withdrawCash(users[userIndex], atmCash, lang ); break;
            case 3: continueSession = depositCash(users[userIndex], lang ); break;
            case 4: continueSession = transferFunds(users[userIndex], users, userCount, lang ); break;
            case 5: continueSession = showTransactionHistory(users[userIndex], lang ); break;
            case 6: continueSession = changePIN(users[userIndex], lang ); break;
            case 7: 
                fullScreenBackground();
...
bool runCardlessSession(User users[], int &userCount, int &atmCash, int lang) {
    fullScreenBackground();
    string accountNumber;
...
bool runAdminSession(User users[], int &userCount, int &atmCash, int lang) {
    int choice;
    do {
        fullScreenBackground();
        if (lang == 1) {
        ...
        switch(choice) {
            case 1: continueAdmin = registerNewUser(users, userCount, lang ); break;
            case 2: continueAdmin = resetUserPIN(users, userCount, lang ); break;
            case 3: continueAdmin = loadATMCash(atmCash, lang ); break;
            case 4: 
                fullScreenBackground();
...
int main() {
    ...
    while (true) {  // MAIN LOOP - keeps returning to language selection
        ...
        do {
            service = chooseService(lang );
            
            if (service == 1) {
                // Card Session - returns false when user exits
                runCardSession(users, userCount, atmCash, lang );
            } else if (service == 2) {
                // Cardless Session - returns false when user exits
                runCardlessSession(users, userCount, atmCash, lang );
            } else if (service == 3) {
                // Admin Session
                if (adminLogin(lang )) {
                    runAdminSession(users, userCount, atmCash, lang );
                } else {
                    fullScreenBackground();
