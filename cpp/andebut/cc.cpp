...
int main() {
    ...
    do {
        ...
        switch(choice) {
            ...
            case 9: cout << GREEN << "Exiting program...\n" << RESET; break;
...
// ================= DATA VALIDATION =================
bool validatePatient(Patient p) {
    if(p.id <= 0) { cout << RED << "ID must be positive!\n" << RESET; return false; }
    if(p.age <= 0 || p.age > 120) { cout << RED << "Invalid age!\n" << RESET; return false; }
    if(p.bmi < 10 || p.bmi > 50) { cout << RED << "BMI seems incorrect!\n" << RESET; return false; }
    if(strlen(p.name) == 0) { cout << RED << "Name cannot be empty!\n" << RESET; return false; }
    if(p.sex != 'M' && p.sex != 'F') { cout << RED << "Sex must be M or F!\n" << RESET; return false; }
    return true;
}
